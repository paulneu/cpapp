        ...
        if (pathHierarchy[pathHierarchy.length - 1] === 'anyOf') {
            ...
            for (const key in propertyList['anyOf']) {
                if (propertyList['anyOf'][key].$ref) {
                    const definitionArray = propertyList['anyOf'][key].$ref.split(common_1.DEFINITION_LINK_PREFIX);
                    ...
                    transferManifestEntriesOVP(currentObject, exportResults, jsonSchema, nextTargetDefinition, factory, [...pathHierarchy]);
                }
                else if (typeof propertyList['anyOf'][key] === 'object') {
                    processProperties(configObject, exportResults, jsonSchema, propertyList['anyOf'][key].properties, factory, [...pathHierarchy, 'anyOfObject']);
                }
            }
        }
    ...
    function getManifestPath() {
        ...
        if (breadcrumbs[0] === 'sections') {
            if (breadcrumbs[1] === 'custom') {
                ...
                path = syncRule.manifest.path(transferParameters.pageKeys, {}, breadcrumbs[1]);
    ...
    if (schemaDefinition && schemaDefinition['properties']) {
        for (const key in schemaDefinition['properties']) {
            ...
            if ((currentConfigObject === undefined || typeof currentConfigObject === 'object') && propertyDefinition) {
                ...
                let nextDefinition = common_1.getNextTargetDefinition(transferParameters.appSchema, title, currentConfigObject, propertyDefinition, key, transferParameters.factory, transferParameters.pageType);
