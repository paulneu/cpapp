                ...
                if (sectionId) {
                    ...
                    for (const selector in flexChanges) {
                        ...
                        if ((!actionId || selector.endsWith(actionId)) &&
                            ((flexChange.propertyId === propertyKey && !flexChanges[selector].facetId) ||
                                (flexChange.propertyId === propertyKey && flexChange.facetId === 'action') ||
                                (flexChange.propertyId === propertyKey &&
                                    flexChange.controlType ===
                                        schemaDefinition['properties'][flexChange.propertyId].controlType))) {
...
function fillFields(importParameters, escapedId, fieldProperties, configPart, factory, pageKeys, flexChanges) {
    ...
    for (const fieldProperty in fieldProperties) {
        ...
        configPart[escapedId]['form']['fields'][fieldProperty] = factory.createInstance(common_3.PageType.ObjectPage, 'DataField');
        transferSettingsAndFlexChanges(importParameters, configPart[escapedId]['form']['fields'][fieldProperty], importParameters.jsonSchema['definitions']['DataField'], pageKeys, flexChanges, undefined, undefined, fieldId);
...
function fillTableInConfig(importParameters, manifestSettings, flexChanges, factory, configPart, pageKeys, sectionId) {
    ...
    const schemaSection = importParameters.jsonSchema['definitions']['ObjectPageSectionTableV2<' + escapedSectionId + '>'];
...
function fillCustomColumnsInConfig(importParameters, escapedSectionId, sectionTitle, configPart, v2Page) {
    ...
    if (customColumnTarget) {
        ...
        listReport_1.addFragments(configPart[escapedSectionId], importParameters.manifest, v2Page, common_3.PageType.ObjectPage, importParameters.fragments, objectPageSectionData);
    }
}
/**
...
function fillSectionsInConfig(importParameters, sectionProperties, manifestSettings, configPart, factory, pageKeys, flexChanges, v2Page, objectPageConfig) {
    ...
    for (const sectionId in sectionProperties) {
        ...
        if (schemaSection['$ref'] &&
            ...
            fillTableInConfig(importParameters, manifestSettings, flexChanges, factory, configPart, pageKeys, sectionId);
            // Custom column ref
            ...
            schemaSection = importParameters.jsonSchema['definitions']['ObjectPageSectionChartV2'];
            sectionTitle = schemaSection.title ? schemaSection.title.split(v2_1.FacetTitlePrefix)[1] : undefined;
            const manifestKey = escapedSectionId.replace('@com.sap', 'com.sap');
            const manifestSection = manifestSettings && manifestSettings.sections && manifestSettings.sections[manifestKey];
            const chartConfig = factory.createInstance(common_3.PageType.ObjectPage, 'ObjectPageChart');
            configPart[escapedSectionId] = { chart: chartConfig };
            if (manifestSection || flexChanges) {
            ...
            schemaSection['$ref'] === common_2.DEFINITION_LINK_PREFIX + 'ObjectPageForm' + '<' + sectionId + '>') {
            const formId = importParameters.jsonSchema['definitions'][`ObjectPageForm<${sectionId}>`]['properties']['form']['$ref'].replace(`${common_2.DEFINITION_LINK_PREFIX}SectionForm`, '');
 ...
 * @param flexChanges - list of flex changes (pre-processed by prepareFlexChanges)
 */
function fillActions(importParameters, actionProperties, configPart, factory, pageKeys, flexChanges) {
    let actionId;
    configPart['actions'] = {};
    if (flexChanges.constructor === Object && Object.keys(flexChanges).length !== 0) {
        for (const actionProperty in actionProperties) {
            ...
            transferSettingsAndFlexChanges(importParameters, configPart['actions'][actionId], importParameters.jsonSchema['definitions']['HeaderAction'], pageKeys, flexChanges, undefined, undefined, actionId);
        }
    }
}
/**
...
function fillConfig(importParameters, pageKeys, flexChangesForTransfer, v2Page, factory) {
    const objectPageConfig = factory.createPageInstance(common_3.PageType.ObjectPage);
...
function createObjectPageConfigV2(importParameters) {
    // Initialize i18next
    i18n_1.initI18n();
    if (!importParameters.manifest[common_3.ManifestSection.generic]) {
        extensionLogger_1.log(importParameters.logger, {
            severity: "error" /* Error */,
    ...
    const pageKeys = [];
    const v2Pages = importParameters.manifest[common_3.ManifestSection.generic].pages;
    if (!v2Pages) {
        extensionLogger_1.log(importParameters.logger, {
            severity: "error" /* Error */,
