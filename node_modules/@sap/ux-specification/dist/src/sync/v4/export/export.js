 ...
 */
function evaluateExportRule(transferParameters, syncRule, breadcrumbs, key, configObject, ids, title, propertyDefinition, nextDefinition) {
    // handle flex change
    if (syncRule.flex) {
        const controlId = syncRule.flex.controlId(transferParameters.baseId, ids, breadcrumbs, syncRule.flex.controlType, title);
 ...
 * @param transferParameters - list of paramters passed from API
 ...
 */
function transferManifestEntriesAndFlexChange(transferParameters, configObject, parentIds, stableIdParts, breadcrumbs, schemaDefinition, title) {
    ...
    if (schemaDefinition && schemaDefinition['properties']) {
        for (const key in schemaDefinition['properties']) {
            ...
            const nextDefinition = utils_1.getNextTargetDefinition(transferParameters.appSchema, title, currentConfigObject, propertyDefinition, key, transferParameters.factory, transferParameters.pageType);
            if (syncRule) {
                evaluateExportRule(transferParameters, syncRule, breadcrumbs, key, configObject, ids, title, propertyDefinition, nextDefinition);
            }
            if (typeof currentConfigObject === 'object' && propertyDefinition) {
                ...
                for (const innerObject of objects) {
                    ...
                    transferManifestEntriesAndFlexChange(transferParameters, innerObject, [...ids, ...additionalIds], localIdParts, [...breadcrumbs, key], nextDefinition.targetDefinition, nextDefinition.title);
                }
            }
        }
    }
